...
def WideResNet_model(depth=28, width=8, dropout_rate=0.0,
                        include_top=False, weights=None,
                        input_tensor=None, input_shape=None,
                        classes=10, activation='relu', attention_module=None):
    ...
    x = __create_wide_residual_network(classes, img_input, include_top, depth, width,
                                       dropout_rate, activation, attention_module)
    ...
    model = Model(inputs, outputs, name='wide-resnet')
...
def __conv1_block(input, attention_module=None):
...
def __conv2_block(input, k=1, dropout=0.0, attention_module=None):
    init = input

    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1

    # Check if input number of filters is same as int(16 * k), else create
    # convolution2d for this input
    if K.image_data_format() == 'channels_first':
        if init._keras_shape[1] != int(16 * k):
            init = Convolution2D(int(16 * k), (1, 1), activation='linear', padding='same')(init)
    else:
        if init._keras_shape[-1] != int(16 * k):
            init = Convolution2D(int(int(16 * k)), (1, 1), activation='linear', padding='same')(init)

    x = Convolution2D(int(16 * k), (3, 3), padding='same')(input)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)



    x = Convolution2D(int(16 * k), (3, 3), padding='same')(x)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
    ...
    if attention_module is not None:
        m = attach_attention_module(m, attention_module)

    return m


def __conv3_block(input, k=1, dropout=0.0, attention_module=None):
    init = input
...
def ___conv4_block(input, k=1, dropout=0.0, attention_module=None):
    init = input
...
def __create_wide_residual_network(nb_classes, img_input, include_top, depth=28,
                                   width=8, dropout=0.0, activation='relu', attention_module=None):
    ...
    x = __conv1_block(img_input, attention_module=attention_module)
    nb_conv = 4

    for i in range(N):
        x = __conv2_block(x, width, dropout, attention_module=attention_module)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = __conv3_block(x, width, dropout, attention_module=attention_module)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = ___conv4_block(x, width, dropout, attention_module=attention_module)
        nb_conv += 2
