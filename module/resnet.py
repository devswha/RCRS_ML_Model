...
__all__ = ['ResNet', 'ResNet50', 'ResNet101', 'ResNet154', 'decode_predictions']
...
def ResNet_model(input_shape=None,
    ...
    x = _create_se_resnet(classes, img_input, include_top, initial_conv_filters,
                          filters, depth, width, bottleneck, weight_decay, pooling, attention_module)
    ...
    model = Model(inputs, outputs, name='resnext')
...
def _resnet_block(input, filters, k=1, strides=(1, 1), attention_module=None):
...
def _resnet_bottleneck_block(input, filters, k=1, strides=(1, 1), attention_module=None):
...
def _create_se_resnet(classes, img_input, include_top, initial_conv_filters, filters,
                      depth, width, bottleneck, weight_decay, pooling, attention_module=None):
    ...
    for i in range(N[0]):
        if bottleneck:
            x = _resnet_bottleneck_block(x, filters[0], width, attention_module=attention_module)
        else:
            x = _resnet_block(x, filters[0], width, attention_module=attention_module)

    # block 3 - N
    for k in range(1, len(N)):
        if bottleneck:
            x = _resnet_bottleneck_block(x, filters[k], width, strides=(2, 2), attention_module=attention_module)
        else:
            x = _resnet_block(x, filters[k], width, strides=(2, 2), attention_module=attention_module)

        for i in range(N[k] - 1):
            if bottleneck:
                x = _resnet_bottleneck_block(x, filters[k], width, attention_module=attention_module)
            else:
                x = _resnet_block(x, filters[k], width, attention_module=attention_module)
