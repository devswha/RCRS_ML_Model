...
def MobileNet_model(input_shape=None,
                alpha=1.0,
                depth_multiplier=1,
                dropout=1e-3,
                include_top=False,
    ...
    x = Activation('relu', name='act_relu')(x)
    outputs = Reshape((classes,), name='reshape_final')(x)


    model = Model(inputs, outputs, name='se_mobilenet_%0.2f_%s' % (alpha, rows))
